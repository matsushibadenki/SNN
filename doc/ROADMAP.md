# **SNN Project Roadmap (v15.2: The Great Unification)**
...
v15.2では、従来のモダリティ別処理（Late Fusion）を撤廃し、**五感を最初からスパイクとして混ぜ合わせる「大統一（The Great Unification）」アーキテクチャ**へと舵を切る。これにより、真に生物学的な「共感覚的知能」の実現を目指す。
...
v15.2以降は、以下の領域に注力し、研究段階から実用段階への移行を図る。
...
**現状の課題:** snn\_researchが肥大化し、依存関係が複雑化。
...
* **Plugin API:** ユーザーが独自の脳領域モジュール（カスタムニューロン、学習則）を追加できるプラグイン機構。snn\_research/plugins/ ディレクトリで管理。  
* **Library Decoupling:** \- neuro-core: SNN基本演算（ニューロン、STDP、シミュレータ）  
...
* **ベンチマークの多様化:** \- DVSデータセット（N-MNIST, DVS-Gesture）での精度  
...
* **Interactive Web Demo:** \- Hugging Face Spaces / Gradioで「眠り、進化し、思考する脳」と対話できるデモを公開  
  * スパイクラスター、Knowledge Graph、Attention Map をリアルタイム可視化  
  * "Ask the Brain" チャットボット（GraphRAGベース）  
* Step-by-Step Tutorials: 1\. "15分でSNN入門" \- MNIST分類  
...
**課題:** プロジェクトの持続可能性と影響力の拡大のために。

**解決策:**

* **Model Zoo & Pre-trained Weights:** \- Hugging Faceに事前学習済みモデルをアップロード（snn-vit-base, snn-gpt-nano等）  
  * ユーザーがFine-tuneできる形式で提供  
* **Neuromorphic Hardware Partnerships:** \- Intel (Loihi), IBM (TrueNorth), BrainChip (Akida) 等とのコラボレーション打診  
  * 彼らのSDKへの対応レイヤーを開発  
* **Community Challenges:** \- "SNN Efficiency Challenge" \- 最も省エネな推論パイプラインの構築コンテスト  
  * "Neuro-Symbolic Reasoning Benchmark" \- 新しい評価指標の提案  
* **Academic Collaboration:** \- 神経科学者、認知科学者とのジョイント研究  
...
### **8\. 🆕 Unified Perception: 五感を切り離さない「液状」統合 (The Liquid Brain)**

**現状の課題:** 従来のAI（Late Fusion）は、画像はCNN、音声はRNNと別々に処理してから結合しており、脳のような「音を聞いて色を感じる」ような低レベルでの連合学習ができない。

**解決策:**

* **Universal Spike Encoding:** 映像(DVS)、音声(MFCC)、テキスト、触覚など、あらゆる入力を統一された\*\*「スパイク列 (Spike Train)」\*\*形式に変換するエンコーダ群を整備。モダリティの違いを「チャネルの違い」として抽象化する。  
* **Liquid Association Cortex (LAC):** 異なるモダリティのスパイクを、物理的に区別せず同じ巨大なリザーバ層（Liquid State Machine）に流し込む。STDP（スパイクタイミング依存可塑性）により、同時に入力された「鐘の音」と「鐘の映像」のニューロン群を物理的に結合させる。  
* **Cross-Modal Plasticity:** 視覚入力が欠損していても、音声入力だけで視覚野のニューロンが発火する（逆も然り）ような、脳特有の補完機能を実装する。

**マイルストーン:**

* Phase 8-6: Universal Spike Encoder (Image/Audio/Text to Spike)  
* Phase 9-9: Liquid Association Cortex (LAC) 実装  
* Phase 9-10: Cross-Modal Association Demo (音で映像を想起するデモ)
...
* **P8-6: Universal Spike Encoder:** あらゆる入力を (Batch, Time, Channels) のスパイク列に変換する統一インターフェースの実装。

**成果物:** 高速化レポート、GSM8K精度、DVSベンチマーク結果、SpikeEncoderモジュール

### **📅 Phase 9: Modular, Distillation & Unified Perception (3-6ヶ月)**
...
* **P9-9: Liquid Association Cortex (LAC):** 視覚・聴覚・言語を混ぜて処理する統合リザーバ層の実装。  
* **P9-10: Cross-Modal Demo:** "Hearing Colors" \- 音声入力のみで視覚野が発火する現象の実証。

**成果物:** pypiパッケージ、Distillationパイプライン、LACデモ、チュートリアル動画
...
* **P11-2: Biological Plausibility Validation:** \- fMRI/EEGデータとSNN活動の相関分析  
  * 神経科学者とのコラボレーション  
  * "Brain-Inspired ≠ Brain-Equivalent" の明確化  
* **P11-3: Catastrophic Forgetting Mitigation:** \- Sleep Consolidation \+ Elastic Weight Consolidation (EWC) のハイブリッド  
  * 継続学習ベンチマーク (CORe50) で95%以上の精度維持を実証  
* **P11-4: Multi-Agent Society Simulation:** \- 複数のSelf-Evolving Agentが協調・競合する環境の構築  
  * 創発的知能の観察と定量化  
* **P11-5: Ethical AI Framework:** \- 倫理的選好 (Ethical Preferences) の理論的基盤を強化  
  * アライメント問題への貢献
...
| 評価軸 | 現在 (v14.1) | 目標 (v15.2+) | Phase 11終了時 | 達成手段 |
| :---- | :---- | :---- | :---- | :---- |
...
| **マルチモーダル** | 個別処理 | **早期統合 (Early Fusion)** | **共感覚的想起** | Liquid Association Cortex |
...
* [**doc/SCALING\_LAW.md**](https://www.google.com/search?q=doc/SCALING_LAW.md) (新規): SNNのスケーリング則に関する実験計画と結果。  
* [**doc/HARDWARE\_GUIDE.md**](https://www.google.com/search?q=doc/HARDWARE_GUIDE.md) (新規): Neuromorphic Hardware対応ガイド。
...
| **五感統合によるノイズ増大** | 中 | 側抑制 (Lateral Inhibition) と注意制御の強化 |
