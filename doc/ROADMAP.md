...
- **Latency**：推論レイテンシ ≤ 10 ms（エッジ限定簡易タスク）
...
8. **Tooling & Distillation Pipeline**
   - Specialist Distillation → SFormer統合
9. **Monitoring & Safety Stack**
   - Explainability Logs, Anomaly Detector, Ethical Guardrails
...
- タスク：
  - Library Decoupling（snn_research を明確に小モジュール化）
  - Unit / Integration Tests の拡充（エネルギー計測の自動化）
  - ベンチマーク環境構築（CIFAR-10, N-MNIST, DVS-Gesture, Speech）
  - Acceptance PR フロー導入

## v16.5（6–12ヶ月） — 蒸留とT=1統合
- **目的**：FrankenMoE→SFormer(T=1)への蒸留パイプライン確立
- タスク：
  - Specialist Distillation Pipeline実装
...
- タスク：
  - Thinking Process（Thinking Tokens）を正式API化
  - Verifier Loop の実装と評価ベンチ
  - GridWorld → 実ロボットの段階的移行
...
- **実装要点**：
  ...
  - 出力API: `encode(input, modality, config) -> SpikeTensor`（ファイル: `snn_research/io/encoder.py`）
- **テスト**：同一概念のクロスモーダル再現性テスト

### B. SFormer (T=1) Backbone
...
### D. Sleep Consolidation / Replay
- **目的**：継続学習と忘却対策
- **実装要点**：
  - Generative Replay（SNNベース生成器）
  - Sleep Scheduler（Astrocyteが時間ウィンドウを制御）
  - Files: `snn_research/memory/sleep.py`

### E. GRPO + Verifier
- **目的**：多段階推論と自己検証
- **実装要点**：
  - 思考トークンをWorking Memoryに書き込むAPI
  - Verifier は別プロセス（低発火率）で結果を検証
  - Verifier 結果による差分学習（local correction patch）を実装

### F. Neuromorphic OS（Astrocyte Network）
...
## C. デモスクリプト（短い流れ）
1. 起動: `python app/unified_perception_demo.py --mode demo`
2. セット: sensor simulator（DVS / audio）をrun
3. 操作: "play audio A" → スクリーンに想起された画像を表示
4. 計測: 消費電力と発火率を収集
5. ログ: `logs/demo_<timestamp>.json`
...
# 12. 参考資料・リンク集（開始点）
- Surrogate gradients survey (Bellec et al.)
- Loihi / SpiNNaker SDK docs
- DeepMind replay / EWC papers
- Active Inference review (Friston)
- Triton performance guide
- Neuromorphic Benchmark Suite（進化中）

---

# 最後に — 実用化への次の3ステップ（具体）
1. **v16.0 を 3ヶ月で固める**：CI/ベンチ/PRフローを整備。Kernel最適化は実験ブランチで。
2. **v16.5 で蒸留実験**：FrankenMoE→SFormer の小スケール蒸留を完遂する。
...
*この文書は v16 提案。編集・ブランチを自由に行ってください。必要ならこの文書を基に v16 の Issues/Projects と Sprint Backlog を作成します。*
