...
python train.py \\  
    \--config configs/smoke\_test\_config.yaml \\  
    \--model\_config configs/models/bit\_rwkv\_micro.yaml \\  
    \--data\_path data/smoke\_test\_data.jsonl \\  
    \--paradigm gradient\_based
...
python train.py \\  
    \--config configs/causal\_learning\_test.yaml \\  
    \--model\_config configs/models/small.yaml
...
python run\_web\_learning.py \\  
    \--topic "量子コンピュータ" \\  
    \--start\_url "\[https://ja.wikipedia.org/wiki/量子コンピュータ\](https://ja.wikipedia.org/wiki/量子コンピュータ)" \\  
    \--max\_pages 3
...
python run\_life\_form.py \--duration 60
...
* **STDP学習 (Unsupervised)**  
  python scripts/run\_stdp\_learning.py \--dataset mnist \--epochs 5

* **Phase 3 統合検証 (SFormer/SEMM)**  
