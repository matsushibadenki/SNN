# ファイルパス: app/main.py
# タイトル: DIコンテナ・Gradio UI起動スクリプト (Thread-Safe & Type Safe Fix)
# 目的: DIコンテナを利用してGradioによるリアルタイム対話UIを起動する。
# 修正: 並行処理時のモデル競合（Race Condition）を解消し、かつmypyの型チェックエラーを修正。
...
class SNNInterfaceApp:
    ...
    def load_inference_services(self, model_id: str) -> Tuple[Optional[ChatService], Optional[ImageClassificationService], str, Dict, Dict, Dict]:
        """
        UIコールバック: 選択されたモデルをロードしてサービスをインスタンス化する (Thread-Safe版)
        DIコンテナのグローバルなオーバーライドを避け、都度Factoryを通じてエンジンを注入する。
        """
        ...
        try:
            ...
            # mypy fix: OmegaConf.merge returns Union[ListConfig, DictConfig], but here we expect DictConfig.
            merged_config = OmegaConf.merge(base_config, {"model": model_config_dict})
            full_config_dict = cast(DictConfig, merged_config)
            
            OmegaConf.update(full_config_dict, "model.path", model_path, merge=True)

            # スレッドセーフな実装: グローバルオーバーライドではなく、新しいエンジンインスタンスを直接生成
            # これにより、ユーザーAとユーザーBが異なるモデルを使っても競合しない
            new_engine = SNNInferenceEngine(config=full_config_dict)
            ...
            if task_type == "text":
                # コンテナのFactoryプロバイダに引数を渡して、エンジンだけ差し替える
                chat_service = self.container.chat_service(engine=new_engine)
                
                if isinstance(chat_service, ChatService):
                    status_message = f"✅ Text Model '{model_id}' loaded (Thread-Safe)."
                ...
                image_service = self.container.image_classification_service(engine=new_engine)
                
                if isinstance(image_service, ImageClassificationService):
                    status_message = f"✅ Image Model '{model_id}' loaded (Thread-Safe)."
