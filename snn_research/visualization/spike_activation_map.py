...
# --- ▼ 修正: cast をインポート ▼ ---
from typing import List, Optional, cast, Union  # Union をインポート
# --- ▲ 修正 ▲ ---
...
class SpikeActivationMap:
    ...
    def __init__(self, model: nn.Module):
        ...
        try:
            ...
            logging.warning(
                "Model has no parameters, defaulting device to CPU for SAM.")
    ...
    def _record_hook(self, module, input, output):
        ...
        if isinstance(output, tuple):
            ...
            # 2番目がスパイクと仮定
            if len(output) > 1 and isinstance(output[1], torch.Tensor) and output[1].numel() > 0:
                spikes = output[1]
            # 1番目がスパイクの場合も考慮
            elif len(output) > 0 and isinstance(output[0], torch.Tensor) and output[0].numel() > 0:
        ...
        if spikes is not None:
            ...
            try:
                ...
                # 特徴量次元のみの場合 (例: 1サンプルの最後の層)
                elif spikes.dim() == 1 and spikes.shape[0] > 0:
                    first_batch_spike = spikes.unsqueeze(
                        0).detach().float().cpu()  # バッチ次元を追加
                    self.spike_history.append(first_batch_spike)
                # else: logging.warning(f"Skipping spike recording due to unexpected shape: {spikes.shape}")
            except Exception as e:
                logging.warning(
                    f"Error processing spikes in hook for module {module}: {e}, Shape: {spikes.shape}")
        # else:
    ...
    def _register_hooks(self, target_layer_name: str):
        ...
        if target_module is not None:
            try:
                # フックを登録
                self.hooks.append(
                    target_module.register_forward_hook(self._record_hook))
                logging.info(
                    f"Registered hook for layer: {target_layer_name} ({type(target_module).__name__})")
                self.target_layer_found = True
            except Exception as e:
                logging.error(
                    f"Failed to register hook for {target_layer_name}: {e}")
        else:
    ...
    def _run_model_forward(self, input_data: torch.Tensor, time_steps: int) -> bool:
        ...
        with torch.no_grad():
            try:
                ...
                logging.info(
                    f"Attempting forward pass with time_steps={time_steps} argument...")
                # SNNコアのインターフェースに合わせて return_spikes=True も渡す
                self.model(input_data, time_steps=time_steps,
                           return_spikes=True)
                if self.spike_history:
                    logging.info(
                        f"  -> Success: Model likely handles time steps internally. Recorded {len(self.spike_history)} spike tensors.")
                    success = True
                else:
                    logging.warning(
                        "  -> Model ran but no spikes recorded via hook. Trying external loop...")
                ...
                if 'time_steps' in str(e):
                    ...
                    for t in range(time_steps):
                        ...
                        # return_spikes=True を試す
                        output = self.model(current_input, return_spikes=True)
                        ...
                        if isinstance(output, tuple):
                            ...
                            if isinstance(next_input_candidate, torch.Tensor):
                                ...
                                logging.error(
                                    f"Output[0] is not a Tensor in external loop at step {t}. Aborting loop.")
                                break
                            # --- ▲ 修正 ▲ ---
                            ...
                            logging.error(
                                f"Unexpected output type {type(output)} in external loop at step {t}. Aborting loop.")
                            break

                    if self.spike_history:
                        logging.info(
                            f"  -> Success: External loop completed. Recorded {len(self.spike_history)} spike tensors.")
                        success = True
                    else:
                        logging.warning(
                            "  -> External loop ran but no spikes recorded via hook.")
                else:
                    # time_steps 以外の TypeError
                    logging.error(
                        f"Model forward pass failed with TypeError (not related to time_steps): {e}")
                    success = False

            except Exception as e_generic:
                logging.error(
                    f"An unexpected error occurred during model forward pass: {e_generic}")
                success = False
    ...
    def _record_spikes(self, input_data: torch.Tensor, target_layer_name: str, time_steps: int) -> List[torch.Tensor]:
        ...
        if not self.target_layer_found:
            logging.error(
                f"Hook not registered for {target_layer_name}. Cannot record spikes.")
            self.clear_hooks()
            return []

        # フォワードパスを実行し、スパイクを記録
        # --- ▼ 修正: 入力データをモデルと同じデバイスに移動 ▼ ---
        spikes_recorded = self._run_model_forward(
            input_data.to(self.model_device), time_steps)
        # --- ▲ 修正 ▲ ---
        ...
        if not spikes_recorded:
            logging.warning(
                f"No spikes were recorded for layer {target_layer_name}. Check hook registration, model forward pass logic, or if the layer actually spiked.")
            return []

        # 記録されたスパイクの形状を確認し、整形
        if recorded_spikes_history:
            logging.info(
                f"Recorded {len(recorded_spikes_history)} spike tensors. Example shape: {recorded_spikes_history[0].shape}")
                ...
                if spikes_at_t.dim() >= 1 and spikes_at_t.shape[0] == 1:
                    ...
                    if spikes_at_t.dim() == 2 and spikes_at_t.shape[1] == expected_features:
                        ...
                        processed_history.append(
                            spikes_at_t.view(1, -1))  # Flattenして追加
                    else:
                        logging.warning(
                            f"Skipping tensor at step {t_idx} due to inconsistent feature dimension. Expected {expected_features}, got shape {spikes_at_t.shape}")
                else:
                    logging.warning(
                        f"Skipping tensor at step {t_idx} due to unexpected batch dimension or shape: {spikes_at_t.shape}")

            if len(processed_history) != len(recorded_spikes_history):
                logging.warning(
                    "Some recorded spike tensors were skipped due to shape inconsistencies.")
            ...
            if num_recorded > 0 and num_recorded < time_steps:
                logging.warning(
                    f"Recorded {num_recorded} steps, expected {time_steps}. Padding with zeros for ISI calculation (might be inaccurate).")
                # 足りない分をゼロで埋める

                padding = [torch.zeros_like(processed_history[-1])
                           for _ in range(time_steps - num_recorded)]
                processed_history.extend(padding)
            elif num_recorded > time_steps:
                logging.warning(
                    f"Recorded {num_recorded} steps, more than expected {time_steps}. Truncating.")
    ...
    def _compute_isi(self, spikes_history: List[torch.Tensor], max_isi: float = 100.0) -> torch.Tensor:
        ...
        try:
            ...
            valid_history = [t.squeeze(0) for t in spikes_history if t.dim(
            ) == 2 and t.shape[0] == 1 and t.shape[-1] == feature_dim]
            if not valid_history:
                logging.error(
                    "Spike history tensors have inconsistent feature dimensions, unexpected batch size, or are empty after filtering.")
                return torch.tensor([])
            # Stack along time dimension
            spikes_tensor = torch.stack(
                valid_history, dim=0)  # (TimeSteps, Features)
        except Exception as e:
            logging.error(
                f"Error stacking spike history: {e}. Check tensor shapes. Example shape: {spikes_history[0].shape}")
            return torch.tensor([])
        ...
        for t in range(time_steps):
            ...
            current_spikes_indices = torch.where(spikes_tensor[t] > 0.5)[
                0]  # Use threshold for robustness
        ...
        avg_isi[valid_mask] = isi_sum[valid_mask] / \
            spike_count_for_isi[valid_mask]

        # スパイクしなかったニューロンも max_isi のまま
        logging.info(
            f"Computed average ISI for {features} neurons. Mean ISI: {avg_isi[valid_mask].mean().item() if valid_mask.any() else 'N/A'}")
        return avg_isi
    ...
    def generate_temporal_heatmap(self, input_data: torch.Tensor, target_layer_name: str, time_steps: int, save_path: Optional[Union[str, Path]] = None):
        ...
        """
        logging.info(
            f"Generating Spike Activation Map for layer '{target_layer_name}'...")
        ...
        spikes_history = self._record_spikes(
            input_data[0:1], target_layer_name, time_steps)
        # --- ▲ 修正 ▲ ---
        ...
        # 2. ISIを計算
        avg_isi_map = self._compute_isi(spikes_history, max_isi=float(
            time_steps + 1))  # 最大ISIをタイムステップ数+1に設定

        if avg_isi_map.numel() == 0:
            logging.error(
                "Failed to compute ISI or ISI map is empty. Cannot generate heatmap.")
            return None
        ...
        if num_features > max_neurons_to_plot:
            indices = torch.linspace(
                0, num_features - 1, max_neurons_to_plot).long()
        ...
        fig, ax = plt.subplots(
            figsize=(max(12, num_features // 50), 3))  # 横幅を特徴量数に応じて調整
        # cmap='viridis_r' もしくは 'plasma_r' など、低い値が目立つカラーマップが良い
        im = ax.imshow(plot_data, cmap='hot_r', aspect='auto',
                       interpolation='nearest', vmin=0, vmax=time_steps + 1)
        fig.colorbar(
            im, ax=ax, label='Average Inter-Spike Interval (ISI)')
        ax.set_title(
            f'Spike Activation Map (SAM) - Layer: {target_layer_name}\nLower ISI (darker color) indicates higher activity')
