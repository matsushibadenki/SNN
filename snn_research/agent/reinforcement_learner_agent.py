...
class ReinforcementLearnerAgent:
    ...
    def get_action(self, state: torch.Tensor, record_experience: bool = True) -> int:
        ...
        with torch.no_grad():
            ...
            if self.use_transformer:
                ...
                logits = out_result['output'] if isinstance(out_result, dict) else out_result
            
            if self.model.training:
                decay_progress = max(0.0, (self.current_lr - self.min_lr) / (self.base_lr - self.min_lr))
    ...
    def _save_checkpoint(self, current_avg: float):
        ...
        self.best_model_state = copy.deepcopy({k: v.cpu() for k, v in self.model.state_dict().items()})
    ...
    def learn_with_grpo(self, trajectories: List[Dict[str, Any]], baseline_reward: float = 0.0):
        ...
        total_rewards = torch.tensor(rewards_list, dtype=torch.float32, device=self.device)
        batch_mean_reward = total_rewards.mean().item()

        if self.current_reward_avg == 0.0:
            self.current_reward_avg = batch_mean_reward
        else:
            self.current_reward_avg = (1 - self.alpha_avg) * self.current_reward_avg + self.alpha_avg * batch_mean_reward
        ...
        if self.use_transformer and self.optimizer is not None:
            ...
            for i, trajectory in enumerate(trajectories):
                ...
                if not episode_history: continue
                
                states = torch.cat([step['state'] for step in episode_history], dim=0) 
                actions = torch.tensor([step['action'] for step in episode_history], device=self.device)
            ...
            for i, trajectory in enumerate(trajectories):
                ...
                adv = float(advantages[i].item())
                clipped_reward = float(np.clip(adv, 0.0, 2.0)) if adv > 0 else float(np.clip(adv, -1.0, 0.0))
                
                episode_history = trajectory.get('spikes_history', [])
                for step_data in episode_history:
                    pre_spikes = step_data.get('pre_spikes')
                    if pre_spikes is None: continue

                    action_idx = step_data['action']
                    target = torch.tensor([action_idx], device=self.device, dtype=torch.long)
