...
from typing import Dict, Any, List, Type, Optional, Union, cast, Tuple
...
class HybridSpikingCNN(BaseModel):
    ...
    def __init__(
        ...
        layer_specific_neurons: Optional[List[str]] = None, # 各層のニューロンタイプ名 ['lif', 'izhikevich', ...]
        **kwargs: Any
    ) -> None:
        super().__init__()
        ...
        types = layer_specific_neurons if layer_specific_neurons else [default_neuron_type] * 3
        if len(types) < 3: types.extend([default_neuron_type] * (3 - len(types)))

        # --- Layer 1 (Conv) ---
        self.features_layers.append(nn.Conv2d(3, 32, kernel_size=3, padding=1))
        self.features_layers.append(self._create_neuron(types[0], 32, default_params))
        self.features_layers.append(nn.AvgPool2d(2))
        
        # --- Layer 2 (Conv) ---
        self.features_layers.append(nn.Conv2d(32, 64, kernel_size=3, padding=1))
        self.features_layers.append(self._create_neuron(types[1], 64, default_params))
        self.features_layers.append(nn.AvgPool2d(2))
        ...
        self.classifier_layers.append(self._create_neuron(types[2], 128, default_params))
    ...
    def _create_neuron(self, type_name: str, features: int, base_params: Dict[str, Any]) -> nn.Module:
        ...
        if type_name == 'lif':
            valid_params = {k: v for k, v in base_params.items() if k in ['tau_mem', 'base_threshold', 'adaptation_strength', 'target_spike_rate', 'noise_intensity', 'threshold_decay', 'threshold_step']}
            return AdaptiveLIFNeuron(features=features, **valid_params)
        elif type_name == 'izhikevich':
            valid_params = {k: v for k, v in base_params.items() if k in ['a', 'b', 'c', 'd', 'dt']}
            return IzhikevichNeuron(features=features, **valid_params)
        elif type_name == 'glif':
            valid_params = {k: v for k, v in base_params.items() if k in ['base_threshold', 'gate_input_features']}
        ...
        **kwargs: Any
        ...
        for t in range(self.time_steps):
            ...
            for layer in self.features_layers:
                out = layer(out)
                if isinstance(out, tuple): out = out[0] # ニューロン出力 (spikes, mem)
        ...
        if outputs_list:
            ...
            out_features = cast(nn.Linear, self.classifier_layers[-1]).out_features
            logits = torch.zeros(B, out_features, device=device)
        
        # 統計
        avg_spikes_val = self.get_total_spikes() / (B * self.time_steps) if return_spikes and self.time_steps > 0 else 0.0
