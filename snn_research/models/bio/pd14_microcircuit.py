...
class PD14Microcircuit(BaseModel):
    def __init__(
        ...
        self.pop_counts = {k: max(10, int(v * scale_factor)) for k, v in base_counts.items()}
        ...
        logger.info(f"🧠 PD14 Microcircuit initialized. Neurons: {sum(self.pop_counts.values())}")
    ...
    def forward(self, thalamic_input: torch.Tensor, topdown_input: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        prev_spikes = {name: torch.zeros(B, count, device=device) for name, count in self.pop_counts.items()}
            ...
            for name, neuron in self.populations.items():
                ...
                for src in prev_spikes:
                    if f"{src}_to_{name}" in self.connections:
                        internal += self.connections[f"{src}_to_{name}"](prev_spikes[src])
                
                ext_soma = torch.zeros_like(internal)
                ext_dend = torch.zeros_like(internal)
                
                if name == "L4e": ext_soma += self.thalamic_input_L4(thalamic_input)
                if name == "L6e": ext_soma += self.thalamic_input_L6(thalamic_input)
                ...
                if isinstance(neuron, TwoCompartmentLIF):
                    spikes, _ = neuron(input_soma=internal+ext_soma, input_dend=ext_dend)
                else:
