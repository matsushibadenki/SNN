...
class SpikingRWKVBlock(sj_base.MemoryModule):
    ...
    def forward(
        ...
        new_time_mixing_state = (time_mixing_state * w) + (k * (1 - w)) * self.time_first
...
class SpikingRWKV(BaseModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # 全ニューロン数計算
        neurons_per_block = cast(SpikingRWKVBlock, self.layers[0]).total_neurons_in_block
        self.total_model_neurons = neurons_per_block * num_layers + d_model * num_layers # blocks + extra mixing

        self._init_weights()
        logger.info(f"✅ SpikingRWKV initialized (Layers: {num_layers}, D: {d_model}).")
    ...
    def forward(self, input_ids: torch.Tensor, return_spikes: bool = False, context_embeds: Optional[torch.Tensor] = None, **kwargs: Any) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        layer_states = [torch.zeros(B, self.d_model, device=device) for _ in range(self.num_layers)]
        ...
        if return_spikes:
            ...
            avg_spikes_val = total_spikes / (B * T_seq * self.total_model_neurons)
        # ------------------------------------
...
class BitSpikingRWKVBlock(sj_base.MemoryModule):
    ...
    def __init__(
        ...
        self.ln_time = SNNLayerNorm(d_model)
        self.time_mix_k = BitLinear(d_model, d_model, bias=False, weight_bits=weight_bits)
        self.time_mix_v = BitLinear(d_model, d_model, bias=False, weight_bits=weight_bits)
        self.time_mix_r = BitLinear(d_model, d_model, bias=False, weight_bits=weight_bits)
        ...
        self.ln_channel = SNNLayerNorm(d_model)
        d_ffn: int = int(d_model * 3.5)
        self.channel_mix_k = BitLinear(d_model, d_ffn, bias=False, weight_bits=weight_bits)
        self.channel_mix_r = BitLinear(d_model, d_model, bias=False, weight_bits=weight_bits)
        self.channel_mix_v = BitLinear(d_ffn, d_model, bias=False, weight_bits=weight_bits)
...
class BitSpikingRWKV(BaseModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        quant_config = model_config.get('quantization', {}) if model_config else {}
        ...
        self.final_norm = SNNLayerNorm(d_model)
        self.output_projection = BitLinear(d_model, vocab_size, weight_bits=weight_bits)
        
        # 全ニューロン数計算
        neurons_per_block = cast(BitSpikingRWKVBlock, self.layers[0]).total_neurons_in_block
        self.total_model_neurons = neurons_per_block * num_layers + d_model * num_layers

        self._init_weights()
        logger.info(f"✅ BitSpikingRWKV initialized (weight_bits={weight_bits}).")
