...
# 日本語タイトル: Spikformer (Phase 2 / BitNet Integrated)
# 目的: BitSpikeLinearを完全統合し、エネルギー効率と精度を両立する。
...
class SpikingSelfAttention(nn.Module):
    """
    Softmax-less SSA with BitNet 1.58bit Linear Layers
    """
    def __init__(self, d_model: int, num_heads: int, tau_m: float = 2.0):
        ...
        self.scale = nn.Parameter(torch.tensor(self.head_dim ** -0.125))

        # BitSpikeLinear に置き換え (ここが重要)
        # SNNへの入力はスパイク(0/1)なので、BitNetの乗算レス効果が最大化される
        self.q_linear = BitSpikeLinear(d_model, d_model)
        self.q_bn = nn.BatchNorm1d(d_model)
        self.q_lif = DualAdaptiveLIFNode(tau_m_init=tau_m, detach_reset=True)

        self.k_linear = BitSpikeLinear(d_model, d_model)
        self.k_bn = nn.BatchNorm1d(d_model)
        self.k_lif = DualAdaptiveLIFNode(tau_m_init=tau_m, detach_reset=True)

        self.v_linear = BitSpikeLinear(d_model, d_model)
        ...
        self.proj_linear = BitSpikeLinear(d_model, d_model)
...
class SpikingMLP(nn.Module):
    def __init__(self, d_model: int, mlp_ratio: int = 4, tau_m: float = 2.0):
        ...
        # BitSpikeLinear に置き換え
        self.fc1 = BitSpikeLinear(d_model, hidden_dim)
        self.bn1 = nn.BatchNorm1d(hidden_dim)
        self.lif1 = DualAdaptiveLIFNode(tau_m_init=tau_m, detach_reset=True)
        
        self.fc2 = BitSpikeLinear(hidden_dim, d_model)
...
class SpikformerBlock(nn.Module):
    def __init__(self, d_model: int, num_heads: int, mlp_ratio: int = 4, init_values: float = 1e-5):
        super().__init__()
...
class Spikformer(BaseModel):
    def __init__(
        self,
        img_size_h: int = 32, # CIFAR default
        img_size_w: int = 32,
        patch_size: int = 4,
        in_channels: int = 3,
        embed_dim: int = 256,
        ...
        # Patch Embedding (Conv2d is standard, keep it or use BitSpikeConv2d if available)
        # Here keeping standard Conv for input layer for stability
        ...
        self.head = BitSpikeLinear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
