...
class MockDVSGenerator(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        # スパース性を模倣 (90%は0)
        spikes = (torch.rand(self.frames_number, self.channels, self.height, self.width) > 0.9).float()
    ...
    def create_dataloader(
        ...
        if use_mock or not SPIKINGJELLY_AVAILABLE:
            ...
            if dataset_name.lower() == 'n-mnist':
                ...
                raise ValueError(f"Unknown dataset: {dataset_name}. Currently only supports 'n-mnist' or Mock.")
