...
class UniversalSpikeEncoder(nn.Module):
    ...
    def _encode_image(self, x: torch.Tensor, method: str) -> torch.Tensor:
        ...
        if method == 'latency':
            ...
            spikes = torch.zeros(batch, self.time_steps, features, device=self.device)
            ...
            spikes = torch.rand_like(x_expanded, device=self.device) < x_expanded
            return spikes.float()

    def _encode_audio(self, x: torch.Tensor, method: str) -> torch.Tensor:
        # x: (B, Time_in, Feat)
        if x.dim() != 3:
             if x.dim() == 4: x = x.flatten(2)
             else: raise ValueError(f"Audio input must be 3D tensor (B, T, F), got {x.shape}")
        ...
        if method == 'delta':
            ...
            diff = torch.cat([torch.zeros(B, 1, num_features, device=self.device), diff], dim=1)
    ...
    def _encode_text_embedding(self, x: torch.Tensor, method: str) -> torch.Tensor:
        ...
        spikes = (torch.rand_like(probs_expanded, device=self.device) < probs_expanded).float()
        return spikes
    ...
    def _encode_dvs(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if T_in != self.time_steps:
            ...
            x_flat = F.interpolate(x_flat, size=self.time_steps, mode='nearest')
            x_flat = x_flat.permute(0, 2, 1)
    ...
    def encode_text_str(self, text: str) -> torch.Tensor:
        ...
        input_ids = torch.tensor(bytes_data, device=self.device, dtype=torch.long)
