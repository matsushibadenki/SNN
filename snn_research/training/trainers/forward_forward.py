...
class SpikingLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if not ((input_dims == 3) or (input_dims == 5)):
            ...
            current = out_static.unsqueeze(1).repeat(1, self.time_steps, *([1]*(out_static.dim()-1)))
        else:
            B, T = x.shape[0], x.shape[1]
            current = self.layer(x.flatten(0, 1)).view([B, T] + list(self.layer(x.flatten(0,1)).shape[1:]))
...
class SpikingForwardForwardLayer(nn.Module):
    def __init__(self, forward_block, threshold=2.0, learning_rate=0.001, time_steps=20):
        ...
        self.optimizer = torch.optim.Adam(self.block.parameters(), lr=learning_rate)
    ...
    def train_step(self, x_pos, x_neg):
        ...
        loss = F.softplus(-(g_pos - self.threshold)).mean() + F.softplus(g_neg - self.threshold).mean()
...
class ForwardForwardLayer(nn.Module):
    def __init__(self, forward_block, threshold=2.0, learning_rate=0.001):
        ...
        # Weight decayを削除
        self.optimizer = torch.optim.Adam(self.block.parameters(), lr=learning_rate)
        self.is_conv = any(isinstance(m, nn.Conv2d) for m in self.block.modules())

    def forward(self, x):
        if self.is_conv:
            norm = x.norm(2, dim=[1,2,3] if x.dim()==4 else [1], keepdim=True) + 1e-8
...
class ForwardForwardTrainer(AbstractTrainer):
    def __init__(self, model: nn.Sequential, device: str = "cpu", config: Optional[Dict[str, Any]] = None, num_classes: int = 10, save_dir: str = "results"):
        ...
        self.ff_layers: List[Union[SpikingForwardForwardLayer, ForwardForwardLayer]] = []
        ...
        for layer in model.children():
            if isinstance(layer, (nn.Linear, nn.Conv2d)):
                ...
                if self.use_snn:
                    ff = SpikingForwardForwardLayer(nn.Sequential(layer), threshold=threshold, learning_rate=lr)
                else:
                    ff = ForwardForwardLayer(nn.Sequential(layer), threshold=threshold, learning_rate=lr)
    ...
    def overlay_y_on_x(self, x, y):
        ...
        if x_mod.dim() == 4: 
            x_mod[:, 0, 0, :min(self.num_classes, x_mod.shape[3])] = y_oh[:, :min(self.num_classes, x_mod.shape[3])]
        elif x_mod.dim() == 2: 
        ...
        for data, target in tqdm(train_loader, desc=f"Epoch {epoch}"):
            ...
            for layer in self.execution_pipeline:
                if isinstance(layer, (ForwardForwardLayer, SpikingForwardForwardLayer)):
                    l, _, _ = layer.train_step(x_pos, x_neg)
                    batch_loss += l
    ...
    def predict(self, test_loader: DataLoader) -> float:
        ...
        with torch.no_grad():
            for data, target in tqdm(test_loader, desc="Predicting", leave=False):
                ...
                goodness_scores = torch.zeros(batch_size, self.num_classes, device=self.device)
                
                for label in range(self.num_classes):
                    y_candidate = torch.full((batch_size,), label, dtype=torch.long, device=self.device)
                    x = self.overlay_y_on_x(data, y_candidate)
                    
                    layer_goodness = torch.zeros(batch_size, device=self.device)
                    current_x = x
                    for layer in self.execution_pipeline:
                        if isinstance(layer, (ForwardForwardLayer, SpikingForwardForwardLayer)):
                            out = layer(current_x)
    ...
    def save_checkpoint(self, filename: str = "checkpoint.pth", metric: Optional[float] = None) -> None:
        ...
        state = {
            ...
            'ff_optimizers': [l.optimizer.state_dict() for l in self.ff_layers],
    ...
    def load_checkpoint(self, checkpoint_path: Union[str, Path]) -> None:
        ...
        if 'ff_optimizers' in state:
            for l, s in zip(self.ff_layers, state['ff_optimizers']):
                l.optimizer.load_state_dict(s)
