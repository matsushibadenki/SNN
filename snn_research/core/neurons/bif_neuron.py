...
# Title: Bistable Integrate-and-Fire (BIF) ニューロンモデル (シミュレーション機能付き)
# Description: 
#   双安定性を持ち、確率的な挙動を示す積分発火ニューロン。
#   単体での挙動検証用メソッド simulate_fluctuations を含む。
#
# 調整内容:
# - matplotlib/numpy がない環境でもメイン機能(forward)が動作するように依存関係を分離。
# - 型ヒントの整合性を確保。
...
if __name__ == "__main__":
    ...
    if not _VISUALIZATION_AVAILABLE:
        ...
        # 5値量子化（例）と経験的エントロピーの計算デモ
        print("\nCalculating empirical entropy for noise=0.08...")
        example = neuron.simulate_fluctuations(n_steps=T, batch_shape=batch_shape, noise_scale=0.08)
        if isinstance(example, dict):
            pots = example["membrane_potentials"].squeeze()
            flat = pots.flatten().numpy()
            
            # スケール: メモリ電位を -1..1 に正規化
            if flat.std() > 0:
                norm = (flat - flat.min()) / (flat.max() - flat.min()) * 2 - 1
            else:
                norm = flat
                
            # 5値レベルへの最近傍量子化
            levels = np.array([-2, -1, 0, 1, 2], dtype=float) / 2.0
            qidx = np.argmin(np.abs(norm[:, None] - levels[None, :]), axis=1)
            
            counts = np.bincount(qidx, minlength=len(levels))
            probs = counts / counts.sum()
            # 0を除外してエントロピー計算
            probs_nonzero = probs[probs > 0]
            emp_entropy = -np.sum(probs_nonzero * np.log2(probs_nonzero))
            
            print("5-level empirical entropy (bits):", emp_entropy)
