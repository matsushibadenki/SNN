...
class OscillatoryNeuronGroup(nn.Module):
    ...
    def __init__(
        ...
        # 位相 (0 ~ 2π)
        self.phase = nn.Parameter(torch.rand(num_neurons) * 2 * 3.14159, requires_grad=False)
        
        # 結合重み行列 (対称行列)
        self.weights = nn.Parameter(torch.randn(num_neurons, num_neurons), requires_grad=False)
        # 自己結合はなし
    ...
    def set_weights(self, weights: torch.Tensor):
        ...
        if weights.shape != (self.num_neurons, self.num_neurons):
            raise ValueError(f"Weights shape mismatch. Expected ({self.num_neurons}, {self.num_neurons})")
