...
class PhaseCriticalHybridCore(nn.Module):
    ...
    def reset_state(self):
        ...
        if self.reservoir_mode == 'phase_critical':
            # cast to access reset_state if using PhaseCriticalSCAL
            # Assuming fast_process has reset_state if phase_critical
...
class HybridNeuromorphicCore(nn.Module):
    ...
    def autonomous_step(
        ...
        if target is None:
            ...
            # 学習ステップ
            metrics = self.core.update_plasticity(
                x_input, target, learning_rate)
