...
# 日本語タイトル: 統合最適化版・1.58ビットロジックゲートレイヤー (Final: Quartic Contrast & Balanced Threshold)
# 内容: 4乗則による最適コントラスト、微弱信号検出用閾値ロジック、MPS対応
...
class LogicGatedSNN(nn.Module):
    ...
    def forward(self, spike_input: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == 'readout':
            # F.normalize (MPS/CUDAでの数値安定性のため eps=1e-6)
            ...
            # --- Quartic Contrast Enhancement (4乗則) ---
            # 5乗則はノイズ0.45以上で信号減衰が激しすぎたため、4乗則(Quartic)に緩和。
            # これによりS/N比を確保しつつ、極限環境下の微弱な正解信号を残す。
            v_mem = cosine_sim.sign() * cosine_sim.abs().pow(4) * 100.0
        else:
        ...
        if self.mode == 'readout':
            ...
            # 4乗則に合わせて係数を0.25に設定。
            # ノイズが高い場合、最大反応(batch_max_v)自体が小さくなるため、
            # 相対閾値を下げて「消え入りそうな正解」を救う。
