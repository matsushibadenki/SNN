# ファイルパス: snn_research/distill/pipeline.py
...
def collate_distillation_batch(batch: List[Dict[str, Any]]) -> Dict[str, torch.Tensor]:
    ...
    padded_inputs = torch.stack([F.pad(x, (0, max_len - x.size(0))) for x in input_ids])
    padded_labels = torch.stack([F.pad(x, (0, max_len - x.size(0))) for x in labels])
    ...
    if 'teacher_logits' in batch[0]:
        ...
        padded_logits = torch.stack([F.pad(x, (0, 0, 0, max_len - x.size(0))) for x in teacher_logits])
...
class AdvancedDistillationPipeline:
    ...
    def __init__(
        ...
        logger.info(f"⚗️ AdvancedDistillationPipeline initialized on {device}.")
    ...
    def run_specialist_distillation(
        ...
        """
        logger.info(f"👨‍🏫 Starting Specialist Distillation with {len(specialist_models)} teachers...")
        
        optimizer = torch.optim.AdamW(self.student_model.parameters(), lr=lr)
        dataloader = DataLoader(unlabeled_dataset, batch_size=batch_size, shuffle=True)
        ...
        for epoch in range(epochs):
            ...
            for batch in pbar:
                ...
                with torch.no_grad():
                    ...
                    # 平均化 (Soft Target)
                    teacher_avg_logits = teacher_logits_sum / len(specialist_models) # type: ignore
            ...
            logger.info(f"✅ Epoch {epoch+1} completed. Avg Loss: {total_loss / len(dataloader):.4f}")
    ...
    def run_cot_distillation(
        ...
        """
        logger.info(f"🧠 Starting CoT Distillation (System 2 -> System 1) with {len(prompts)} prompts...")
        ...
        for prompt in tqdm(prompts, desc="Reasoning Rollout"):
            ...
            result = reasoning_engine.think_and_solve(input_ids, task_type="general")
            
            if result['final_output'] is not None:
                # 思考過程と思考結果を結合したテキストを作成
                # thought_trace はリストなので結合
                thought_text = " ".join(result['thought_trace']) # 実際にはもっと詳細なCoTが必要
                ...
                cot_data.append({
                    ...
                    'labels': full_ids.squeeze(0)     # Causal LM training (next token prediction)
                })
        ...
        for epoch in range(epochs):
            total_loss = 0.0
            pbar = tqdm(dataloader, desc=f"CoT Distill Epoch {epoch+1}/{epochs}")
        ...
        logger.info("✅ CoT Distillation complete. System 1 has absorbed System 2's reasoning patterns.")
