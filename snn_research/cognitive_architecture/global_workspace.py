...
# 日本語タイトル: グローバル・ワークスペース (DI対応修正版)
# 修正: __init__ に model_registry 引数を追加し、**kwargs で柔軟性を持たせる。
...
class GlobalWorkspace:
    ...
    def conscious_broadcast_cycle(self):
        ...
        winner = max(self.blackboard.items(), key=lambda x: x[1]["salience"])[0]
        content = self.blackboard[winner]["data"]
        
        self.conscious_broadcast_content = content
        self.stream_of_consciousness.append({"source": winner, "content": content})
