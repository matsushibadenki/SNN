...
# 日本語タイトル: Surprise-Gated Brain Kernel v20.1
...
class SurpriseGatedBrain(AsyncArtificialBrain):
    ...
    async def process_event(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not is_safe:
            await self.astrocyte.consume_energy("REFLEX_REJECTION")
        ...
        if prediction_error > self.surprise_threshold:
            ...
            if await self.astrocyte.request_compute_boost():
                # mypy修正: 既存の think_and_solve を呼び出す
                ...
                await self.astrocyte.log_fatigue(0.15)
            else:
                final_output = f"Fallback (Energy Low): {s1_output}"
        else:
            final_output = s1_output
            await self.astrocyte.consume_energy("SYSTEM1_IDLE")
