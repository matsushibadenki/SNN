...
class ReasoningEngine:
    def __init__(
        ...
        logger.info(
            "🧠 ReasoningEngine v2.6 initialized (Repetition Penalty Enabled).")
    ...
    def think_and_solve(
        ...
        force_system2 = any(w in input_text for w in [
                            "calculate", "code", "python", "計算", "書いて", "足し算"])
            ...
            for attempt in range(self.max_retries + 1):
                    ...
                    if codes:
                        ...
                        if not success or "Error" in out:
                            ...
                            generated_ids = torch.cat(
                                [generated_ids, result_ids], dim=1)
                ...
                with torch.no_grad():
                    if hasattr(self.model, 'embedding'):
                        ...
                        score = self.verifier(
                            hidden).item() + code_score_modifier
                    else:
                        score = 0.5
                ...
                if is_correct or attempt == self.max_retries:
                    ...
                    if correction_prompt:
                        corr_ids = tokenizer.encode(
                            correction_prompt, return_tensors='pt').to(self.device)
                        current_input_ids = torch.cat(
                            [generated_ids, corr_ids], dim=1)

        best = max(candidates, key=lambda x: x["score"])
        final_text = tokenizer.decode(
            best["output_ids"][0], skip_special_tokens=True) if tokenizer else ""
