...
# (Phase 3: Multimodal Dataset Support)
# Title: データセット定義
# Description:
...
class ImageTextDataset(SNNBaseDataset):
    """
    画像パスとキャプション（テキスト）のペアを読み込むデータセット。
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        if image_path:
            # 相対パスの場合はデータセットファイルのディレクトリを基準にする
            ...
            try:
                ...
                pixel_values = torch.zeros(3, 224, 224) # ダミー画像
        else:
            pixel_values = torch.zeros(3, 224, 224)

        # 教師あり学習用 (Next Token Prediction)
        return {
