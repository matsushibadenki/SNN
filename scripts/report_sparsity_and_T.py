...
def measure_efficiency(model_config_path: str, data_path: str, model_path: Optional[str] = None, vocab_size: int = 50257):
    ...
    if model_path:
        if os.path.exists(model_path):
            ...
            try:
                ...
                if state_dict is not None:
                    try:
                        ...
                        logger.info("✅ Model weights loaded successfully (exact match, strict=False).")
                    except RuntimeError as e:
                        logger.warning(f"Standard load failed ({e}). Attempting prefix adjustment...")
                        ...
                        try:
                            ...
                            logger.info("✅ Model weights loaded successfully with prefix adjustment (strict=False).")
                        except RuntimeError as e2:
                            logger.error(f"❌ Prefix adjustment also failed: {e2}")
                            # 最後の手段: 内部モデルへ直接ロード
                            if hasattr(model, 'model'):
                                logger.info("Attempting direct load into inner model...")
                                model.model.load_state_dict(state_dict, strict=False)
                                logger.info("✅ Model weights loaded directly into inner model (strict=False).")
                            else:
                                raise e2
            ...
            logger.warning(f"⚠️ Model path provided but file not found: {model_path}")
    # --- ▲ 追加 ▲ ---
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vocab_size", type=int, default=50257, help="Vocabulary size for the model (default: 50257 for GPT-2)")
    args = parser.parse_args()
    
    measure_efficiency(args.model_config, args.data_path, args.model_path, args.vocab_size)