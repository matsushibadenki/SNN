...
class SCALFeatureExtractor(nn.Module):
    ...
    def fit(self, data_loader, epochs=1):
        self.scal.train()
        print(f"initializing Visual Cortex (SCAL)...")
        for _ in range(epochs):
            for data, _ in data_loader:
                if not data.is_contiguous(): data = data.contiguous()
...
class BrainVisualizer:
    ...
    def setup_model(self):
        ...
        # 1. 視覚野 (SCAL)
        self.feature_extractor = SCALFeatureExtractor(784, 1000, n_models=3, device=self.device)
        ...
        self.snn_layers = [
            SpikingForwardForwardLayer(layer1, threshold=0.15, time_steps=30).to(self.device),
            SpikingForwardForwardLayer(layer2, threshold=0.15, time_steps=30).to(self.device)
        ]
    ...
    def train_quick(self, loader):
        ...
        # SNN学習
        for layer in self.snn_layers: layer.train()
        ...
        for data, target in pbar:
            ...
            y_fake = (target + torch.randint(1, 10, (len(target),)).to(self.device)) % 10
    ...
    def visualize(self, loader):
        ...
        for i, layer in enumerate(self.snn_layers):
            ...
            print(f"Layer {i+1} Spikes captured. Active neurons: {np.sum(spike_raster) / spike_raster.size * 100:.2f}%")

        # --- Plotting ---
        plt.style.use('dark_background')
        fig = plt.figure(figsize=(15, 10))
        fig.suptitle(f"Brain Activity for Input '{label}'", fontsize=16, color='white')
        ...
        sns.heatmap(feature_vec.reshape(25, 40), cmap='viridis', ax=ax2, cbar=False)
        ...
        ax3.imshow(raster1[active_indices], aspect='auto', cmap='binary', interpolation='nearest')
        ...
        ax4.imshow(raster2[active_indices2], aspect='auto', cmap='magma', interpolation='nearest')
