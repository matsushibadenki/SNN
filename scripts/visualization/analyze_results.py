...
def analyze_hpo_results(
    ...
    try:
        ...
        # (sqlite3 がない場合など)
        logger.error(
            f"Could not load study. Ensure database driver is installed.")
        return
    ...
    try:
        ...
        fig_history = optuna.visualization.plot_optimization_history(
            study)  # type: ignore[attr-defined]
...
def show_tensorboard_instructions() -> None:
    ...
    logger.info(
        "3. Open the URL provided in your browser (e.g., http://localhost:6006/).")
    logger.info("---------------------------------------------")
