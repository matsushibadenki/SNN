    ...
    def hook(module: nn.Module, input_tensor: Tuple[torch.Tensor], output: Any) -> None:
        ...
        if spk_tensor is not None and mem_tensor is not None:
            with torch.no_grad():
                ...
                thr_data = threshold_tensor.detach().cpu() if isinstance(threshold_tensor, torch.Tensor) else None
...
def run_dynamics_visualization(config_path: str, timesteps: int, output_path_str: str) -> None:
    ...
    try:
        model = ModelRegistry.get_model(cast(DictConfig, model_config_dict), load_weights=False)
        model.eval()
    ...
    BATCH_SIZE = 4
    dummy_input = create_dummy_input(cast(DictConfig, model_config_dict), timesteps=timesteps, batch_size=BATCH_SIZE)
    ...
    try:
        ...
        if spikes_stack.dim() >= 2:
            ...
            while spike_counts.dim() > 1: spike_counts = spike_counts.sum(dim=-1)
            # --- 修正: int() キャストを追加 ---
        ...
        logger.info(f"✅ 最も活動的なサンプル (Batch Index {best_batch_idx}) を選択しました (Spikes: {int(max_spikes)})。")
...
def main() -> None:
    ...
    parser.add_argument('--output_path', type=str, default="workspace/runs/dynamics_viz/dynamics.png")
    args = parser.parse_args()

    if not os.path.exists(args.model_config): sys.exit(1)
    run_dynamics_visualization(args.model_config, args.timesteps, args.output_path)
