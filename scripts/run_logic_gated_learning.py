...
def run_simulation():
    ...
    # [修正] カリキュラム: Hyper-Contrast Boosting v2
    # 以前の0.47までの学習はノイズが強すぎて逆に精度を破壊していたため、
    # 上限を0.46に抑えつつ、ターゲットとなる0.45付近の学習密度を高めました。
    curriculum_stages = [
        {'range': (0.0, 0.30), 'epochs': 10, 'lr': 0.1},
        {'range': (0.2, 0.40), 'epochs': 10, 'lr': 0.05},
        {'range': (0.35, 0.45), 'epochs': 15, 'lr': 0.02}, 
        # 修正: 重要区間のエポック数を20->30に増やし、地盤を固める
        {'range': (0.40, 0.46), 'epochs': 30, 'lr': 0.005}, 
        # 修正: 最終段階の範囲を(0.44, 0.47)から(0.42, 0.46)に変更。
        # 0.47の破壊的なノイズを除去し、0.45前後の識別能力を最大化する。
        # LRも0.0005だと収束が遅すぎる可能性があるため0.001へ微増。
        {'range': (0.42, 0.46), 'epochs': 40, 'lr': 0.001}, 
    ]
