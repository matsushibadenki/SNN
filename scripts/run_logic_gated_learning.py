...
# 日本語タイトル: 統合最適化・自律学習シミュレーション (Fix: 詳細ログ出力版)
...
def run_simulation():
    ...
    print("\nStarting Readout Training Phase (Detailed Metrics)...")
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (data, target) in enumerate(loader):
            ...
            if i % 50 == 0:
                ...
                # ログ出力: Loss, リザーバー発火率(%), 出力発火率(%), 膜電位平均, 膜電位最大
    ...
    for noise in noise_levels:
        ...
        with torch.no_grad():
            ...
            for data, target in test_loader:
                # 評価時も autonomous_step を呼ぶが学習はさせない手もあるが、
                # ここでは純粋な forward + 手動計測を行う
                out = core(data)
                ...
                # V_Mean (LogicGatedSNNに保存された最新バッチの値を取得)
        ...
        if noise == TRAIN_NOISE and final_acc > 95.0: status = "Excellent"
