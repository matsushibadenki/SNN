...
class SynestheticAgent:
    def __init__(self):
        ...
        if os.path.exists(ckpt_path):
            self.brain.load_state_dict(torch.load(ckpt_path, map_location=self.device), strict=False)
            print("✅ Synesthetic Circuits (Vision-Language) Loaded.")
        else:
            print("⚠️ Warning: Synesthesia checkpoint not found. Vision will be random.")

        # 3. 推論エンジンの構築
        self.gen_adapter = SNNGenerativeAdapter(self.brain.core_brain, device=self.device)
        self.astrocyte = AstrocyteNetwork(initial_energy=1000.0, max_energy=1000.0, recovery_rate=10.0)
        self.rag = RAGSystem(embedding_dim=256)
        
        self.rag.add_knowledge("Brain v4.0 possesses artificial synesthesia, allowing it to see and feel concepts.")
    ...
    def see_image(self, image_path: str) -> str:
        ...
        try:
            ...
            img_tensor = self.transform(img).view(-1).to(self.device) # Flatten [784]
            
            start_token = self.tokenizer.encode("Digit", return_tensors="pt")[0,0].item()
            ...
            description = self.tokenizer.decode(gen_ids, skip_special_tokens=True)
        ...
        try:
            ...
            for img, label in mnist:
                if label == digit:
                    ...
                    img.save(filename)
                    print(f"   [🖼️ Image Created] Saved '{filename}'. You can see it with '/image {filename}'")
                    return
        except Exception as e:
    ...
    def chat(self, user_input: str):
        ...
        if "error" in result:
            ...
            final_text = result.get("final_text", "").replace(prompt, "").strip()
