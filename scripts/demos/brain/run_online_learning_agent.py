...
class OnlineAgent:
    def __init__(self):
        ...
        print(f"🧠 Initializing Online Learning Brain on {self.device.upper()}...")
    ...
    def _generate_mock_sensory_data(self, modality: str, pattern_name: str):
        ...
        if modality == 'vision':
            ...
            if "fall" in pattern_name: # 落下
                ...
                for t in range(8):
                    tensor[0, t, 0, 14:, 10:18] = torch.rand((14, 8)).to(self.device)
            else:
    ...
    def learn_concept(self, text_context: str, target_concept: str, 
        ...
        # コンテキストと正解ラベルの準備
        inputs = self.tokenizer(text_context, return_tensors="pt").input_ids.to(self.device)
        target_id = self.tokenizer.encode(" " + target_concept, return_tensors="pt")[0, 0].to(self.device)
    ...
    def chat_loop(self):
        ...
        print("   /teach [concept]      : Teach the brain what the sensation is")
        print("   (Normal text)         : Chat with the brain")
        print("-------------------------------------------------------\n")
        
        # 状態保持用
        current_sensation = {"img": None, "tac": None, "olf": None}
        last_sensation_desc = "Nothing"
        ...
        while True:
            try:
                ...
                if user_input.startswith("/"):
                    ...
                    if cmd == "/see":
                        tensor, desc = self._generate_mock_sensory_data('vision', arg)
                        current_sensation["img"] = tensor
                        last_sensation_desc = desc
                        print(f"   {desc}")
                        
                    elif cmd == "/touch":
                        tensor, desc = self._generate_mock_sensory_data('tactile', arg)
                        current_sensation["tac"] = tensor
                        last_sensation_desc = desc
                        print(f"   {desc}")
                        
                    elif cmd == "/smell":
                        tensor, desc = self._generate_mock_sensory_data('olfactory', arg)
                    ...
                    with torch.no_grad():
                        logits = self.brain(
                            text_input=self.tokenizer(prompt, return_tensors="pt").input_ids.to(self.device),
                        ...
                        print("   (If wrong, type '/teach [correct_word]' to learn)")
                    ...
                    with torch.no_grad():
                        ...
                        logits = self.brain(
                            text_input=inputs,
                            image_input=current_sensation["img"], # コンテキストとして感覚を保持
                            tactile_input=current_sensation["tac"],
                            olfactory_input=current_sensation["olf"]
                        )
                        pred_id = torch.argmax(logits[:, -1, :]).item()
