...
def run_plasticity_demo():
    ...
    logger.info(f"   ✂️ Pruned: {stats['pruned']} synapses (Weak connections removed)")
    logger.info(f"   🌱 Grown:  {stats['grown']} synapses (New random connections created)")
    ...
    if strong_connections_mean > weak_connections_mean:
        logger.info("✅ SUCCESS: Important knowledge preserved while structure evolved.")
    else:
        logger.warning("⚠️ WARNING: Rewiring might have damaged important knowledge.")
