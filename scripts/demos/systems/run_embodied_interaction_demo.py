...
def run_embodied_demo():
    ...
    try:
        ...
        logger.warning(f"Registry build failed ({e}), falling back to direct init.")
    ...
    dummy_prompt = torch.tensor([[101, 20, 30, 40]], device=device) # [CLS, "What", "do", "?"]
    ...
    if len(generated_text) > 0 and len(action_values) == motor_config["action_dim"]:
        logger.info("✅ Multimodal Integration (Vision -> Language + Motor) Verified!")
    else:
