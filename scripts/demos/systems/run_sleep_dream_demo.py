...
def run_sleep_demo():
    ...
    sleeper = SleepConsolidator(agent, optimizer, buffer_size=50, batch_size=4, device=device)
    ...
    if improvement > 0.001: # 微小な誤差以上の改善
        logger.info(f"✅ Memory Consolidated! Loss improved by {improvement:.4f}")
    else:
        logger.warning(f"⚠️ Memory consolidation result inconclusive (Diff: {improvement:.4f}).")
