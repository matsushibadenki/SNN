...
class SimpleEnvironment:
    ...
    def __init__(self, img_size=32):
        ...
        self.state = torch.randn(1, 3, img_size, img_size) # Current visual state
...
def run_intrinsic_demo():
    ...
    dummy_text = torch.randint(0, vocab_size, (1, 8)).to(device) # Context/Goal (Fixed for now)
    ...
    for step in range(steps):
        ...
        logger.info(f"   Step {step}: Loss={metrics['loss']:.4f}, PredErr={metrics['prediction_error']:.4f}, Reward={metrics['intrinsic_reward']:.4f}")
