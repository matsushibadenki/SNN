...
def demo_test_time_adaptation():
    logger.info("\n=== 1. TTA Demo ===")
    config = {"architecture_type": "spiking_cnn", "num_classes": 10, "time_steps": 16, "neuron": {"type": "lif"}}
    model = SNNCore(config, vocab_size=10)
    bcm = BCMLearningRule(0.01, 50.0, 0.1)
    tta_model = TestTimeAdaptationWrapper(model, bcm, adaptation_rate_multiplier=0.5)
    ...
    with torch.no_grad():
        ...
        for i in range(3):
            ...
            logger.info(f"Noisy Step {i+1} Output Mean: {out[0].mean().item():.4f} (Adapting...)")

def demo_bio_pc_network():
    logger.info("\n=== 2. Bio-PCNet Demo ===")
    pc_net = BioPCNetwork([128, 64, 10], 16, {"type": "lif", "tau_mem": 10.0}, 0.005)
