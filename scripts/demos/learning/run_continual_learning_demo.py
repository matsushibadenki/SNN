...
class SCALFeatureExtractor(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad(): out = self.scal(x_flat)
...
class ContinualTrainer(ForwardForwardTrainer):
    ...
    def train_sleep_phase_with_replay(self, batch_size=64):
        ...
        """
        for layer in self.execution_pipeline: layer.train()
        
        # バッファからサンプル (CPUにある状態)
        features_mem, labels_mem = self.replay_buffer.sample(batch_size)
        if features_mem is None: return 0.0
        ...
        y_fake = (labels_mem + torch.randint(1, self.num_classes, (len(labels_mem),)).to(self.device)) % self.num_classes
...
def run_continual_learning_demo():
    ...
    fashion_train = datasets.FashionMNIST(data_dir, train=True, download=True, transform=transform)
    ...
    scal_dim = 1000
    # 5つのモデルで多様な特徴を捉える
    scal = SCALFeatureExtractor(in_features=784, out_features=scal_dim, n_models=5, device=device)
    ...
    trainer = ContinualTrainer(feature_extractor=scal, model=snn_model, device=device, config=config, num_classes=10)
    ...
    for epoch in range(1, 6):
        ...
        # Sleep & Replay (記憶の定着)
        for _ in range(50): trainer.train_sleep_phase_with_replay()
    ...
    for epoch in range(1, 6):
        ...
        # 睡眠時間を増やして定着を強化
        for _ in range(100): trainer.train_sleep_phase_with_replay()
