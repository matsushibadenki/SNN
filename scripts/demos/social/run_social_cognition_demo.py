...
class ActorAgent:
    ...
    def get_trajectory(self):
        ...
        return torch.tensor(traj, dtype=torch.float32).unsqueeze(0) # [1, 16, 2]
...
def run_social_demo():
    ...
    for episode in range(50):
        ...
        for t in range(20):
            ...
            if t > 5:
                ...
                target_tensor = torch.tensor(target, dtype=torch.float32).to(device).unsqueeze(0)
        ...
        if (episode+1) % 10 == 0:
            logger.info(f"   Episode {episode+1}: Prediction Loss = {loss.item():.4f}")
    ...
    for t in range(15):
        ...
        # Display
        status = "🤔 Guessing..."
        if dist < 0.1: status = "💡 I KNOW!"
        
        logger.info(f"   Step {t:02d}: Pos={actor.pos.round(2)} -> Predicted Goal={pred_pos.round(2)} | Err={dist:.2f} | {status} ({lat:.2f}ms)")
