...
class SCALFeatureExtractor(nn.Module):
    def __init__(self, in_features, out_features, n_models=3, device='cpu'):
        ...
        self.scal = EnsembleSCAL(in_features=in_features, out_features=out_features, n_models=5).to(device)
        self.device = device
...
class HippocampalReplayBuffer:
    ...
    def push(self, features, label):
        for i in range(features.size(0)):
            self.buffer.append((features[i].detach().clone(), label[i].detach().clone()))
...
class BioReplayTrainer(ForwardForwardTrainer):
    ...
    def train_sleep_phase(self, batch_size=64, noise_intensity=1.0, replay_ratio=0.5):
        ...
        noise_input = torch.randn(noise_size, 1000).to(self.device) * noise_intensity
        dummy_labels = torch.randint(0, self.num_classes, (noise_size,)).to(self.device)
        x_noise = self.overlay_y_on_x(noise_input, dummy_labels)
        
        if len(self.replay_buffer) > replay_size:
            features_mem, labels_mem = self.replay_buffer.sample(replay_size)
            features_mem, labels_mem = features_mem.to(self.device), labels_mem.to(self.device)
            x_mem_pos = self.overlay_y_on_x(features_mem, labels_mem)
            y_mem_fake = (labels_mem + torch.randint(1, self.num_classes, (len(labels_mem),)).to(self.device)) % self.num_classes
...
def run_scal_spiking_ff_fashion():
    ...
    scal_dim = 1000
    # モデル数を増やして複雑なパターンに対応
    scal_extractor = SCALFeatureExtractor(in_features=784, out_features=scal_dim, n_models=5, device=device)
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        for _ in range(sleep_iters):
            sleep_loss_accum += trainer.train_sleep_phase(batch_size=64, noise_intensity=1.0, replay_ratio=0.5)
    ...
    save_path = os.path.join(os.path.dirname(__file__), '../../../../results/checkpoints/spiking_ff_fashion.pth')
