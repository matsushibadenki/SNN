...
class SocialAgent:
    ...
    async def think(self, topic: str):
        ...
        result = self.brain.run_cognitive_cycle(topic)
...
async def main():
    ...
    num_agents = 3
    # 合意形成エンジン
    consensus_engine = ConsensusEngine(num_agents=num_agents, proposal_dim=16, device=device)
    # 文化リポジトリ
    ...
    confidences_tensor = torch.tensor(confidences, device=device).unsqueeze(1) # [Num_Agents, 1]
    ...
    if status == "AGREED":
        ...
        logger.info(f"Trust updated. {best_agent.name} gained significant reputation.")
