...
def main():
    ...
    logger.info(f"   - Defined {len(concepts)} concepts and {len(rules)} logic rules.")
    ...
    # 1. 記号接地 (Grounding)
    detected_symbols = ns_bridge.extract_symbols(neural_features, threshold=0.5)
    ...
    logger.info(f"   -> Feedback Signal Strength: {feedback_signal.norm().item():.4f}")
