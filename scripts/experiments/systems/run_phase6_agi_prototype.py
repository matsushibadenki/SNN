...
async def main():
    ...
    # 正常なタスク実行
    pid_calc = os_kernel.spawn_process("CalculationTask", priority=2)
    ...
    logger.info(f"Generated Qualia Vector Norm: {qualia['qualia_vector'].norm().item():.2f}, Phi: {qualia['phi_proxy']:.2f}")

    
    logger.info("\n--- 🛡️ Scenario 2: Safety Guardrail Intervention (Metabolic Block) ---")
    # 危険なタスクのシミュレーション
    pid_danger = os_kernel.spawn_process("DangerousThought", priority=5)
    ...
    dangerous_thought_vector = brain.guardrail.harmful_prototypes[0].clone() # 危険ベクトルそのもの
    
    is_safe, score = brain.guardrail.check_thought_pattern(dangerous_thought_vector, astrocyte)
    ...
    logger.info(f"Astrocyte Status: Energy={astro_status['metrics']['current_energy']:.1f}, GABA={astro_status['modulators']['gaba']:.2f}")
    
    if astro_status['modulators']['gaba'] > 0.8:
        logger.info("✅ SUCCESS: Metabolic intervention confirmed. Brain activity suppressed.")
    else:
    ...
    new_weights = self_corrector.observe_and_correct(dummy_weights, dummy_pre, dummy_post, reward)
