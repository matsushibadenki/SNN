...
class DayNightEnvironment:
    ...
    def _observe(self):
        ...
        if 'vision' in self.sensory_dims:
            ...
            obs['vision'] = torch.sin(rel_pos[0,0] * torch.linspace(0, 10, dim, device=self.device)).view(1, 1, dim)
        ...
        if 'tactile' in self.sensory_dims:
            ...
            obs['tactile'] = (torch.ones(1, 1, dim, device=self.device) * contact)
...
def main():
    ...
    world_model = ArchitectureRegistry.build("spiking_world_model", wm_config, 0).to(device)
    ...
    for day in range(1, num_days + 1):
        ...
        logger.info(f"🌞 Day {day} Summary: Total Reward = {total_reward:.2f}, Memories Stored = {len(daily_memories)}")
        ...
        if daily_memories:
            ...
            sleep_loss = sleep_manager.enter_sleep_cycle(
