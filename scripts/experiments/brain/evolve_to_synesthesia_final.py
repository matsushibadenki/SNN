...
class SynesthesiaDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if isinstance(label, torch.Tensor): digit = label.item()
        else: digit = label
...
def train_synesthesia():
    ...
    if os.path.exists(base_ckpt):
        print(f"📥 Loading base chat capability...")
        brain.core_brain.load_state_dict(torch.load(base_ckpt, map_location=device), strict=False)
    else:
    ...
    torch.save(brain.state_dict(), "models/checkpoints/brain_v4_synesthesia.pth")
    ...
    gen_ids = brain.generate(
        ...
        start_token_id=tokenizer.encode("Digit", return_tensors="pt")[0,0].item(),
        max_new_tokens=20
    )
