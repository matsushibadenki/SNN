...
class FastConversationalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if end > len(self.data):
            ...
            chunk = torch.cat([chunk, torch.full((pad_len,), self.tokenizer.eos_token_id, dtype=torch.long)])
        else:
...
def train():
    ...
    print(f">>> Configuration: Layers={CONFIG['num_layers']}, TimeSteps={CONFIG['time_steps']}", flush=True)
    ...
    state_dict = {k: v for k, v in model.state_dict().items() if "spikes" not in k and "mem" not in k}
    ...
    try:
        with torch.no_grad():
            ...
            for _ in range(10):
                ...
                if next_tk.item() == tokenizer.eos_token_id: break
