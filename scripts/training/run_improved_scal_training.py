...
def run_experiment(use_ensemble: bool = False, use_multiscale: bool = True):
    ...
    curriculum = [
        {'noise': 0.10, 'epochs': 5, 'lr': 0.02, 'batch': 2048, 'mix': False, 'mix_ratio': 0.0},
        {'noise': 0.30, 'epochs': 5, 'lr': 0.02, 'batch': 4096, 'mix': False, 'mix_ratio': 0.0},
        {'noise': 0.40, 'epochs': 10, 'lr': 0.015, 'batch': 4096, 'mix': True, 'mix_ratio': 0.2},
        {'noise': 0.45, 'epochs': 40, 'lr': 0.01, 'batch': 4096, 'mix': True, 'mix_ratio': 0.1},
        {'noise': 0.48, 'epochs': 20, 'lr': 0.01, 'batch': 4096, 'mix': True, 'mix_ratio': 0.1},
    ]
    ...
    for stage in curriculum:
        ...
        stage_name = f"Noise {noise_level:.2f}" + (f" (Mix {mix_ratio})" if use_mix else "")
    ...
    for noise in test_noise_levels:
        ...
        elif acc_mean > 0.95: status = "Goal Met (>95%)"
        elif acc_mean > 0.90: status = "Good"
        elif acc_mean > 0.80: status = "Acceptable"
        
        if noise == 0.45 and acc_mean >= 0.90: status = "TARGET ACHIEVED ✓✓"
        elif noise == 0.45 and acc_mean >= 0.80: status = "IMPROVED ✓"
