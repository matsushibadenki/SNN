...
def main() -> None:
    ...
    parser.add_argument("--output_dir", type=str, default="benchmarks/continual_learning")
    parser.add_argument("--benchmark_script", type=str, default="scripts/run_benchmark_suite.py")
    
    # --- ▼ 修正: デフォルトパスを scripts/runners/train.py に変更 ▼ ---
    parser.add_argument("--train_script", type=str, default="scripts/runners/train.py") 
    # --- ▲ 修正 ▲ ---
    ...
    for model_type, model_path, tag in [("EWC", task_b_ewc_path, "EWC"), ("Finetune Only", task_b_finetune_path, "Finetune")]:
        for task, task_tag in [("Task A (SST-2)", "TaskA"), ("Task B (MRPC)", "TaskB")]:
            ...
            results_data.append({"Model": model_type, "Task": task, "Accuracy": acc})

    # --- Stage 4 ---
    print("\n" + "="*20 + " Stage 4: Report " + "="*20)
    df = pd.DataFrame(results_data)
    try:
        df_pivot = df.pivot(index="Model", columns="Task", values="Accuracy").reset_index()
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        f.write(f"# Continual Learning Report\n\n{df_pivot.to_markdown(index=False)}\n\n{conclusion}")
