...
# (修正: Check 10 のインラインコードで cfg.model を渡すように修正)
...
def run_command(command, description):
    ...
    try:
        # capture_output=True で出力を捕捉し、エラー時に表示
...
def main():
    ...
    # ダミーANNが必要だが、convert_model.py がダミー生成に対応していればOK
...
# 修正: cfg.model があればそれを使う
model_params = OmegaConf.to_container(cfg.model if 'model' in cfg else cfg, resolve=True)
model = SNNCore(model_params, vocab_size=100)
...
"""
    ...
    # --- 12. Agent (Task Solver) ---
    results["12. 自律エージェント (Task Solver)"] = run_command(
        [python_cmd, "scripts/runners/run_agent.py", "--task_description", "health_check_task", "--force_retrain",
         "--model_config", "configs/models/micro.yaml", "--unlabeled_data_path", "data/smoke_test_data.jsonl"],
        "12. 自律エージェント (Task Solver)"
    )

    # --- 13. Planner ---
    results["13. 階層的プランナー (Planner)"] = run_command(
        [python_cmd, "scripts/runners/run_planner.py", "--task_request", "Plan a health check sequence", 
         "--context_data", "System status is unknown"],
        "13. 階層的プランナー (Planner)"
    )

    # --- 14. Multimodal Demo ---
    results["14. マルチモーダル処理デモ"] = run_command(
        [python_cmd, "scripts/run_multimodal_demo.py"],
        "14. マルチモーダル処理デモ"
    )

    # --- 15. Spatial Demo ---
    results["15. 空間認識デモ"] = run_command(
        [python_cmd, "scripts/run_spatial_demo.py"],
        "15. 空間認識デモ"
    )

    # --- 16. ECG Analysis ---
    results["16. ECG異常検知デモ"] = run_command(
        [python_cmd, "scripts/run_ecg_analysis.py", "--model_config", "configs/models/ecg_temporal_snn.yaml", "--num_samples", "2", "--time_steps", "100"],
        "16. ECG異常検知デモ"
    )

    # --- 17. HPO ---
    results["17. ハイパーパラメータ最適化 (HPO)"] = run_command(
        [python_cmd, "scripts/runners/run_hpo.py", "--model_config", "configs/models/micro.yaml", "--task", "health_check",
         "--n_trials", "1", "--eval_epochs", "1", "--metric_name", "accuracy", "--output_base_dir", "runs/health_check_hpo"],
        "17. ハイパーパラメータ最適化 (HPO)"
    )

    # --- 18. Spike Viz (Patterns) ---
    results["18. スパイク活動可視化 (Patterns)"] = run_command(
        [python_cmd, "scripts/visualize_spike_patterns.py", "--model_config", "configs/models/micro.yaml", 
         "--timesteps", "8", "--batch_size", "2", "--output_prefix", "runs/health_check_viz"],
        "18. スパイク活動可視化 (Patterns)"
    )

    # --- 19. Spike Viz (Dynamics) ---
    results["19. ニューロンダイナミクス可視化"] = run_command(
        [python_cmd, "scripts/visualize_neuron_dynamics.py", "--model_config", "configs/models/micro.yaml",
         "--timesteps", "8", "--output_path", "runs/health_check_dynamics.png"],
        "19. ニューロンダイナミクス可視化"
    )
    ...
    if passed_count != total_count:
        logger.error(f"⚠️ {total_count - passed_count} 個のチェックが失敗しました。詳細は上記のログを確認してください。")
        sys.exit(1)
    else:
        logger.info("🎉 すべてのチェックが合格しました！")
        sys.exit(0)
