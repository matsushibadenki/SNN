...
def register_hooks(model: nn.Module):
    ...
    for name, layer in model.named_modules():
        if isinstance(layer, TARGET_LAYERS):
            ...
            logger.info(f"フック登録: {name} ({type(layer).__name__}) | {param_info}")
...
def create_dummy_input(model_config: DictConfig, batch_size: int, timesteps: int) -> torch.Tensor:
    ...
    if isinstance(input_shape, (list, List, type(OmegaConf.create([])))) and len(input_shape) == 3:
        ...
        logger.info(f"シーケンス型ダミー入力作成: (B, T)")
...
def run_spike_test(config_path: str, timesteps: int, batch_size: int):
    ...
    dummy_input = create_dummy_input(model_config_dict, batch_size, cli_seq_len)
    reset_spike_counts()
    ...
    for name in layer_names:
        ...
        spike_rate = (spikes / possible_spikes) * 100.0 if possible_spikes > 0 else 0.0
    ...
    print("="*60)
    overall_rate = (total_spikes_all / total_possible_all) * 100.0 if total_possible_all > 0 else 0.0
