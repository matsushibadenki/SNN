...
class MNIST_SpikingCNN(BaseModel):
    def __init__(self, num_classes=10, time_steps=4, neuron_config=None):
        ...
        if neuron_config is None:
            neuron_config = {'features': 0, 'tau_mem': 2.0, 'base_threshold': 1.0}
    ...
    def forward(self, x, return_spikes=False):
        ...
        for m in self.modules():
            if hasattr(m, 'set_stateful'): m.set_stateful(True)
        ...
        for m in self.modules():
            if hasattr(m, 'set_stateful'): m.set_stateful(False)
...
def main():
    ...
    model = MNIST_SpikingCNN(num_classes=10, time_steps=4, neuron_config=neuron_config).to(device)
    ...
    avg_spike_rate = sum(spike_rates) / len(spike_rates) if spike_rates else 0.0
    
    logger.info(f"🏆 Result: Acc={acc:.2f}%, Latency={avg_latency:.2f}ms, SpikeRate={avg_spike_rate:.1%}")
    ...
    logger.info("➡️ Now you can run: python scripts/verify_performance.py --metrics_json results/best_mnist_metrics.json")
