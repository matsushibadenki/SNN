...
"""
ファイルパス: snn-cli.py
タイトル: SNN Project CLI Tool (v16.2 - Enhanced Cleanup)
機能説明:
SNNプロジェクトの主要なワークフロー（学習、評価、変換、UI起動、最適化、診断など）を
統一的に管理するためのコマンドラインインターフェース（CLI）ツール。
修正: cleanコマンドの対象に logs, data/prepared, __pycache__ を追加。
"""
...
def _perform_clean(delete_models=False, delete_data=False, yes=False):
    # 基本ターゲット
    targets = ['runs', 'workspace', 'results', 'logs']
    
    # データ削除フラグがある場合
    if delete_data:
        targets.extend(['precomputed_data', 'data/prepared'])
    
    # 削除から保護する拡張子
    protected_extensions = {'.md'}
    if not delete_data: 
        protected_extensions.update({'.jsonl', '.json', '.db', '.csv', '.txt'}) # .txtも念のため保護
    ...
    # 2. __pycache__ の収集 (再帰的)
    if delete_data or delete_models: # data/model削除時、または明示的なオプションがあればキャッシュも消す
        for root, dirs, files in os.walk("."):
            for d in dirs:
                if d == "__pycache__":
                    pycache_dir = os.path.join(root, d)
                    # ディレクトリごと削除リストへ（後で処理）あるいは中のファイルを削除リストへ
                    for pyc_file in glob(os.path.join(pycache_dir, "*")):
    ...
    # __pycache__ も対象に含める
    dirs_to_check = targets + [x[0] for x in os.walk(".")] 
...
def ui_start(start_langchain, model_config, model_path, **kwargs):
    """UI起動"""
    if model_path:
        verify_path_exists(model_path, "モデルファイル", "学習済みモデルのパスを確認するか、先に学習を実行してください。")
...
def evaluate_accuracy(model_path, model_config, model_type, experiment, tag):
    ...
    if "converted" in model_path:
        suggestion = f"モデルが見つかりません。変換コマンドを実行しましたか？\n例: python snn-cli.py convert ann2snn-cnn {experiment}_ann.pth {model_path} --snn-model-config {model_config}"
    else:
